 ...
 */
 export const alertObj = function (obj) {
  ...
  export const removeByValue = function (arr, attr, value) { // 1数组 2属性 3属性值
    var index = -1
    for (var i in arr) {
      if (arr[i][attr] === value) {
        index = i
  ...
  // 树形过滤
  export const fileterTree = function (data, child, prop, val) { // 树形数组  子节点名称  判断字段  字段值
    let list = []
  ...
  export const check = ['String', 'Array', 'Number', 'Object', 'Boolean', 'Function', 'Null', 'Undefined'].reduce((res, type) => {
    ...
    get: function (key) {
      ...
      if (this.obj) {
        ...
        if (result.substr(0, 6) === '<json>') {
          ...
          // 返回数组
          var obj = JSON.parse(result.slice(7)); var arr = []
          for (var item in obj) {
            arr.push(obj[item])
          }
          return arr
